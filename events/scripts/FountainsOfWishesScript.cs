...
public partial class FountainsOfWishesScript : EventScript
{
    public override IReadOnlyList<EventOptionData> GetOptions()
    {
        ...
        var option1 = new EventOptionData("Offer 50 coins (Receive a Common Relic)", EventOptionValue.FromInt(0), gold < 50 || !commonAvailable);
        var option2 = new EventOptionData("Offer 80 coins (Receive a Rare Relic)", EventOptionValue.FromInt(1), gold < 80 || !rareAvailable);
        var option3 = new EventOptionData("Offer 120 coins (Receive a Epic Relic)", EventOptionValue.FromInt(2), gold < 120 || !epicAvailable);
