...
var selected_tower_configuration: TowerConfigurationWithInstance = null
var selected_item: ChooseTowerScreenItem = null


static func show_screen(configurations: Array[TowerConfigurationWithInstance], canvas: CanvasLayer) -> void:
...
func populate_screen(configurations: Array[TowerConfigurationWithInstance]) -> void:
	for tower_configuration in configurations:
		var item = TOWER_ITEM_SCENE.instantiate() as ChooseTowerScreenItem
		item.tower_configuration = tower_configuration
...
func _on_ok_pressed() -> void:
	...
	ClickEvents.add_tower_card.emit(selected_item.tower_configuration)
	done.emit()
	queue_free()
